...
```{r}
pdf(file=file.path("plots","surface_truth.pdf"))
par(mfrow=c(1,4))
par(mar=c(0,1,0,1))
...
pdf(file = file.path("plots","surface_regression.pdf"))
par(mfrow=c(4,6))
par(mar=c(0,0,0,0))
for(i in 1:4) {
  ...
  # BARS
  binbars = BinBARS(x_train[,1],x_train[,2],y_obs,burns=500,effes=500,intercepts=c(T,T))
  contour(x=grid,y=grid,z=outer(grid,grid,function(x,y) {return(binbars_fit(x,y,binbars))}),axes = FALSE,frame.plot =TRUE)
  # Thin Plate Spline
  model_tps = Tps(x_train,y_obs,m=2) 
  contour(x=grid,y=grid,z=outer(grid,grid,function(x,y) {return(predict(model_tps,cbind(x,y)))}),axes = FALSE,frame.plot =TRUE)
}
dev.off()
time.end = Sys.time()
