...
PME=function(x.obs, d, projection_map, N0=20*D, tuning.para.seq=exp((-15:5)), alpha=0.05, max.comp=100, epsilon=0.05, max.iter=100, print.MSDs=FALSE){
  ...
  for (tuning.ind in 1:length(tuning.para.seq)) {
    ...
    Fitting[[tuning.ind]]=proj.points
  ...
  fitted.opt=Fitting[[optimal.ind]]
  eta.func=function(t){
    eta.func.prepare=function(tau){ return(eta.kernel(t-tau,lambda)) }
