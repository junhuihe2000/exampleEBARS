...
for(rep in 1:n_exp) {
  ...
  cat("broken stick noisy samples with dim (", dim(xobs), ")\n")
  write.csv(t(xobs), file=file.path(fileroot, "dataset", mani, paste0(rep,".csv")))
  ...
  print(t2-t1)
  write.csv(t(x.ebars), file=file.path(fileroot, "output", mani, paste0("tsme_trial",rep,".csv")))
  ...
  print(t2-t1)
  write.csv(t(x.pme), file=file.path(fileroot, "output", mani, paste0("pme_trial",rep,".csv")))
  ...
  print(t2-t1)
  write.csv(t(x.pc), file=file.path(fileroot, "output", mani, paste0("pc_trial",rep,".csv")))
}
t.end = Sys.time()
```
